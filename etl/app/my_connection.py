...
def connect_to_redis():
    # redis_host = get_environment_var()['redis_host']
    redis_host = get_environment_var()['redis_host']
    redis_port = int(get_environment_var()['redis_port'])
    red_conn = Redis(host=redis_host, port=redis_port, db=0)
    state = State(RedisStorage(red_conn))
    return state
